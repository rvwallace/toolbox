...
# sesh zsh completion bootstrap

toolbox_require_commands sesh sesh || return 0

_toolbox_sesh_load_zsh_completion() {
    ...
    cache_file="${cache_dir}/_sesh"
    version_file="${cache_dir}/sesh.version"
    current_version="$(sesh --version 2>/dev/null | tr -d '\r')"
    [[ -n "$current_version" ]] || return 0

    toolbox_completion_cache_ensure "$cache_file" "$version_file" "$current_version" sesh completion zsh || return 0
